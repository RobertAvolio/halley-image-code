...
c
      ...
      twopi=pi*2.0
      conv=pi/180.0
      write(*,'("give image name to be created: ",$)')
      read(*,*) imnew
      print*,'give heliocentric distance to the comet in AU'
      read*,helio
      print*,'give geocentric distance to the comet in AU'
      read*,didist
      print*,'give pixel scale in arcsec per pixel'
      read*,scale
      pix=725.3*scale*didist
      print*,'give cometographic longitude and latitude'
      print*,'of the center of the active region in degrees'
      read*,cenlon,cenlat
      cenlon=cenlon*conv
      cenlat=cenlat*conv
      ccenla=cos(cenlat)
      scenla=sin(cenlat)
      print*,'give radius of the active region in degrees'
      print*,'[must be less than or equal to 90 deg]'
      read*,raddeg
      radar=raddeg*conv
      cradar=cos(radar)
      print*,'give std. deviation for the Gaussian dispersion of'
      print*,'parents/grains (in the radial direction) in degrees'
      print*,'[gaussian distribution which assumes a flat cross'
      print*,' section truncates at a radial distance of 90 deg]'
      print*,' suggested std. deviation:  < 30 deg'
      read*,sigjet
      sigjet=sigjet*conv
      constr=1.0-exp(-0.125*(pi/sigjet)**2)
      print*,'give long. and lat. of the angular momentum'
      print*,'vector in the cometocentric ecliptic system'
      print*,'in degrees'
      read*,angra,angdec
      angra=angra*conv
      angdec=angdec*conv
      cangde=cos(angdec)
      sangde=sin(angdec)
      print*,'give long. and lat. of the solar direction'
      print*,'in the cometocentric ecliptic system'
      print*,'in degrees'
      read*,solra,soldec
      solra=solra*conv
      soldec=soldec*conv
      ...
      ztail=-ssolde
      print*,'give long. and lat. of the Earth direction'
      print*,'in the cometocentric ecliptic system'
      print*,'in degrees'
      read*,earra,eardec
      earra=earra*conv
      ...
      searra=sin(earra)
c
      print*,'give number of apertures (<6) for aperture photometry'
      read*,numap
      do 100 n=1,numap
      print*,'give aperture radius in pixels for aperture number',n
      print*,'[note: aperture radius must be > seeing]'
      read*,rad(n)
...
c
      ...
      print*,'give angle between the angular momentum vector'
      print*,'and the polar axis in degrees [0 for PA rotation]'
      read*,angthe
      angthe=angthe*conv
      cangth=cos(angthe)
      sangth=sin(angthe)
      print*,'give rotation period P_psi in days'
      print*,'[set to a large number if PA rotation]'
      print*,'Note: ratpsi>0 for LAM'
      read*,ratpsi
      ratpsi=twopi/(ratpsi*86400.0)
c    
      print*,'give precession period P_phi in days'
      print*,'[set to rotation period if PA rotation]'
      read*,ratphi
      ratphi=twopi/(ratphi*86400.0)
      print*,'give the time of observation in days '
      print*,'from reference time (> 0 and < 10)'
      read*, date
c
      timfin=date
      print*,'give the corresponding Euler angle for rotation'
      print*,'at reference time deg.'
      read*,gpsi
      gpsi=gpsi*conv
      print*,'give the corresponding Euler angle for precession'
      print*,'at reference time in deg.'
      read*,gphi
      gphi=gphi*conv
c
      beta=grav*solmas/au/au
c
      print*,'give radiation pressure acceleration in anti sunward'
      print*,'direction in cm/sec/sec for a parent/grain at 1 AU'
      print*,'beta = 1 is', beta,' cm/sec/sec'
      read*,radg
      radg=radg*(1.0e-5)/(helio**2)
      print*,'give radiation pressure acceleration in anti'
      print*,'sunward direction in cm/sec/sec for a radical at 1 AU'
      print*,'beta = 1 is', beta,' cm/sec/sec'
      read*,radr
      radr=radr*(1.0e-5)/(helio**2)
      print*,'give the mean parent/grain velocity in km/sec'
      read*,velgr
...
c     read*,delvg
c     delvg=0.5*delvg
      print*,'give radical velocity in km/sec'
      read*,velr
      print*,'give the exponential decay time for the daughter'
      print*,'production rate or the time required to achieve the'
      print*,'maximum daughter production rate (ie growth time)'
      print*,'in sec (latter case assumes a linear growth and'
      print*,'linear decay with equal growth and decay times'
      print*,'for the daughter production rate)'
      print*,'Look at the code to determine which case is used'
      read*,lifgra
      lifgra=lifgra/(helio**2)
      print*,'give life time of the radical in sec'
      read*,lifrad
      lifrad=lifrad/(helio**2)
      print*,'give time bin size in sec'
      read*,timbin
      print*,'give the maximum time jet action to be followed in sec'
      read*,totime
      num=nint(totime/timbin)
      print*,'give the number of locations in the active region to be'
      print*,'uniformly sampled (per time bin) [suggestion: > 100]'
      read*,nregion
      print*,'give the maximum number of parent/grain particles to be'
      print*,'emitted per sampled location (per time bin)'
      print*,'[suggestion: > 100]'
      read*,binnum
      print*,'give the constant background production rate as a'
      print*,'fraction of the maximum production rate'
      read*,bg
c 
